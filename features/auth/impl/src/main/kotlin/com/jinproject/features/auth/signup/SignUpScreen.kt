...
internal fun SignUpScreen() {
    ...
    val uiState = rememberSignUiState()
    val navigator = LocalNavigator.current

    LaunchedEffect(uiState) {
        uiState.sideEffect.collectLatest { effect ->
            when (effect) {
                is SignSideEffect.ShowSnackBar -> showSnackBar(effect.message)
                is SignSideEffect.NavigateAfterSignInSuccess -> navigator.popTo(AuthRoute.SignIn, true)
            }
        }
    }
            ...
            TextButton(
                ...
                onClick = {
                    ...
                    coroutineScope.launch {
                        signUiState.signUp(context = context)
                    }
                },
            )
        }

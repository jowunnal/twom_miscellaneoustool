<a href="https://play.google.com/store/apps/details?id=com.jinproject.twomillustratedbook">
	<img src="https://img.shields.io/badge/PlayStore-v2.5.6-4285F4?style=for-the-badge&logo=googleplay&logoColor=white&link=https://play.google.com/store/apps/details?id=com.jinproject.twomillustratedbook" />
</a>
...
- **몬스터 도감** : 사냥터를 기준으로 해당 지역의 출몰 몬스터를 상세 조회 · 검색 · 필터링
- **아이템 도감** : 카테고리(무기, 방어구, 잡템, 코스튬) 기준으로 분류된 아이템을 상세 조회 및 필터링
- **몬스터 알람** : 아이모 내 보스 몬스터들의 재생성 시간을 계산하지 않고 편리하게 알람을 생성
- **현재시간 항상 보기 기능** : "다른앱 위에 그리기" 를 통해 현재시간과 등록된 몬스터 알람 시간을 선택적으로 함께 노출
- **길드 마크 변환 기능** : 앱 외부 저장소의 이미지 파일을 가져와 갤러리 형태로 노출하고, 사용자가 선택한 이미지를 12*12 픽셀의 길드 마크 심볼로 변환
- **AI 길드 마크 이미지 생성 기능** : Dalle3 REST API 를 활용하여, 로그인 한 유저가 최적화된 프롬프트로 길드 마크로 변환할 AI 이미지 생성
- **강화 시뮬레이터** : 랜덤 수치의 스텟을 가진 무기, 방어구 아이템 생성 후, Drag-And-Drop UX 로 강화 시뮬레이션
- **로그인 및 회원 관리** : Firebase-Authentication 을 이용하여 로그인 및 회원관리를 제공하고, 로그인 한 유저를 대상으로 인앱결제가 필요한 'AI 길드 마크 이미지 생성' 및 '길드 마크 변환' 결과를 Firebase-Storage 를 이용하여 백업 기능을 제공
...
- 사용자가 점진적으로 늘어나고 이에 따라 기능을 확장하는 과정에서, 작동이 되지 않는 버그들이 발생했습니다.
...
### As-Is
- 직접 몬스터의 (죽은 시간 + 재 생성까지 걸리는 시간) 을 계산하고, 시스템 알람 앱에 설정하는 과정이 번거롭습니다.
- 또한, 계산 실수가 발생할 가능성이 큽니다.

### Challenge
- 알람 생성
  - 몬스터의 재 생성 시간은 최대 7일 까지 걸릴 수 있고, 앱이 **백그라운드**에 있거나 디바이스가 **도즈모드**에 진입해도 **정시** 에 울려야 합니다.
  - 따라서 **AlarmManager#setAlarmClock** 으로 알람을 생성하고, **BroadcastReceiver** 에서 수신하여 **Notification** 을 발생시키도록 구현했습니다.
- 알람 재 생성
  - 알람 재 생성 로직은 Service 로 분리하고, Notification 에서 Service 를 시작시키는 "알람 재생성" Action을 추가합니다.
    - 왜 Service 인가?
      - **BroadcastReceiver** 는 상대적으로 **작업을 수행하는데 오랜 시간이 소요되는**  I/O 나 CPU-consuming 한 작업을 실행하기 적합하지 않습니다.
      - Scheduling API 의 경우,  Notification은 별도의 프로세스인 **NotificationManagerService** 에서 intent 를 실행하기 때문에 **PendingIntent** 가 필수적이기 때문에 사용할 수 없습니다.
      - ForegroundService 는 백그라운드에서 실행할 경우, **ForegroundServiceStartNotAllowedException** 이 발생하기 때문에 사용할 수 없습니다.
      - 따라서 알람 데이터 저장 및 재설정 작업은 별도의 **백그라운드용 Service** 로 데이터 전달 후 실행합니다.
- 단위테스트 작성
  - 알람 생성 로직에 대해 kotlin 언어 기반 라이브러리인 **mockk** 로 주입하고, **kotest** 이용하여 단위테스트 작성 및 실행

결과적으로 몬스터의 죽은 시간만 입력하면 간편하게 몬스터의 재 생성 시간 계산한 뒤에 알람으로 설정해주고, 등록된 알람 리스트를 간편하게 노출해주는 기능을 구현했습니다.

### To-Be
- 안드로이드에서의 백그라운드 제약(배터리 절약, 눈에 보이지 않아 사용자가 실행 중 인지 알 수 없는 문제)을 이해하고, 백그라운드 작업 가이드를 기반으로 올바른 API 를 선택할 수 있습니다.
- 앱의 핵심 기능에 대한 단위테스트의 중요성을 깨닫았고, 테스트 가능한 코드를 작성하기 위한 [의존성 주입 개념](https://jowunnal.github.io/android/%EC%9D%98%EC%A1%B4%EC%84%B1_%EC%A3%BC%EC%9E%85/)에 대해 학습했습니다.
...
<details>
<summary>이미지를 길드 마크 심볼(12*12 칸의 픽셀) 색상 팔레트로 변환 하는 기능 구현</summary>
<div markdown="1">

### As-Is
- 기기 내부 혹은 외부의 이미지들을 길드 마크 심볼(12*12 칸의 픽셀) 색상 팔레트로 변환하는 기능을 구현 해야 했습니다.
  - 디바이스 내부이면서, 앱 외부의 저장소에 존재하는 이미지 파일
  - AI 이미지 생성형 기능을 통해 생성된 이미지의 download url
  - Firebase-Storage 에 저장된 이미지의 download url
- 모바일 인게임 환경에서 사용할 수 있도록, Overlay(다른 앱 위에 그리기) 로 제공 해야 했습니다.

### Challenge
- 이미지의 비트맵 객체 가져오기
  - Content Uri
    - **READ_MEDIA_IMAGES** 와 안드로이드 14 버전 이상 이라면 추가로 **READ_MEDIA_VISUAL_USER_SELECTED** 에 대한 권한을 요청합니다.
  ...
  - Http Url
    - 이미지의 uri 가 "http" 문자로 시작하는지 확인합니다.
    - 해당하는 경우, **Coil#ImageLoader** 로 요청하여 이미지를 가져온 뒤, 변경 가능하게 하기 위해 비트맵 타입을 Software 로 변환합니다.
- 12*12 픽셀의 이미지 변환
  - 가져온 비트맵을 **Bitmap#createScaledBitmap** 을 이용하여 12*12 픽셀 로 변환합니다.
  - 변환된 비트맵을 **Bitmap#getPixels** 을 이용하여 색상 배열을 추출하고, 비슷한 **색상들을 공통화** 하여 노출합니다.
    - 비슷한 색상들을 공통화 하는 이유는 UX를 고려하여 육안으로 구분하기 힘들 정도의 비슷한 색상들이 "색상 팔레트" 에 나뉘어져 존재하는 문제 때문입니다.
    - **색상 공통화 알고리즘**
      - "색상 팔레트" 로 담을 리스트를 생성
      - 12*12 크기의 변환된 색상 배열에 대해 완전 탐색
        - 해당 색상과 "색상 팔레트" 리스트 내의 색상들과의 rgb 값 차이가 "색상 정밀도" 수치 범위 내에 있다면, 리스트에 추가하지 않고 반복을 종료
        - 색상 리스트의 끝까지 없다면, 리스트에 추가
      - 마지막으로 "색상 팔레트" 리스트를 rgb 값 순서대로 정렬하여 반환

정리하자면, 이미지는 Android#Bitmap 으로 변환하여 처리하며, 내부적으로 12*12 픽셀 형태로 변환하는 등의 **픽셀 처리**가 필요하므로 변경 가능한 **Software 타입의 Bitmap** 객체를 이용하는 것이 핵심입니다.

### To-Be
- **Bitmap(래스터)** 과 **Vector(백터)** 이미지 파일 포맷의 종류인 **png, jpg, webp, svg** 에 대한 장단점 및 차이를 이해할 수 있었습니다.
...
- 사용자의 기기 내부 저장소에 저장된 이미지들을 가져와서 갤러리 형태로 노출해야 했습니다.
- 시스템 갤러리 앱과 같이 **무한 스크롤** 가능하고, **스크롤 바** 가 존재하는 갤러리 UI/UX 를 제공하려 했습니다.
  - 개별 이미지는 **확대될 수 있어야 하고**, 클릭 시 상세 이미지 화면으로 전환 필요

### Challenge
- 커스텀 갤러리
  - why?
    - 안드로이드 14 버전 부터 앱 외부의 저장소로 부터 이미지를 가져오는 권한에 대한 제한이 강화되면서 특별한 사용사례가 아니면 **Photo Picker** 를 사용하는 것이 강제되고 있습니다.
    - 하지만 "길드 마크 심볼 생성" 기능은 앱의 핵심 기능이고, 사용자의 접근이 빈번하게 이루어질 수 있으며, 이미지에 대한 변환을 수행하는 기능이 제공되기 때문에 READ_MEDIA_IMAGES 권한을 사용합니다.
    - 또한 Dynamic 한 UI Component 를 개발하고자 하는 목적이 있었으므로 **Photo Picker** 대신 **커스텀 갤러리**를 구현하는 방법을 선택했습니다.
  - How?
    - [Scrollable Layout](https://github.com/jowunnal/twom_miscellaneoustool/blob/master/design/src/main/kotlin/com/jinproject/design_compose/component/lazyList/ScrollableLayout.kt) 컴포넌트 구현
      - LazyList 의 (한개의 view height * 이미지 개수) 로 **스크롤 바의 위치**를 계산하여 표시
      ...
      - "최상단으로 이동하기" 버튼과 스크롤바는 **[코루틴을 활용한 타이머](https://github.com/jowunnal/twom_miscellaneoustool/blob/master/design/src/main/kotlin/com/jinproject/design_compose/component/lazyList/TimeScheduler.kt)**로 3초간 스크롤이 발생하지 않으면 자동으로 사라지도록 구현
- 무한 스크롤
  - User Interaction 은 기본적으로 Root View 에서 최하위 Leaf View 까지 전달된 후, **Leaf View 에서 부터 소비**하고, **소비되지 않은 interaction 을 상위 view 가 소비**할 수 있습니다.
    - 따라서, 갤러리 컴포넌트는 Root > Scrollable Layout > LazyList(Gallery) > GalleryItem 순으로 interaction 을 소비합니다.
    - 이를 이용하여 LazyList 에서 **소비되지 않은 스크롤**이 발생하면, 다음 페이지를 가져오도록 구현했습니다.
- 갤러리 이미지 페이징
...
- Configuration Change 가 발생해도 스크롤과 같은 상태를 유지하기 위해 ViewModel 과 rememberSaveable 을 이용하여, 적절한 상태 관리 방법들을 적용하였습니다.
...
### As-Is
- 기존 코드를 리펙토링 할 때 마다, 단위테스트 실행을 별도로 진행해 주어야 했습니다.
- 개발이 끝나고 릴리즈를 위해 Master 브랜치에 push 할 때 마다, 반복되는 작업들을 일일이 수행해 주어야 했습니다.
  - versionCode 를 증가한 뒤, 릴리즈 빌드로 aab 생성
  - 생성된 aab를 플레이콘솔에 배포
  - Release 노트 작성
  - readme 에 노출되는 버전들 최신화
- 이러한 과정들을 실수로 수행하지 않아, 누락되는 일들이 빈번하게 발생했습니다.

### Challenge
- Github Actions 를 활용하여 Develop 브랜치에 CI 환경 구축
  - 신규 기능이나 코드 작성이 수행한 뒤에, Develop 에 push 할 때 마다 반복되는 작업(lint 검사, 테스트 실행)을 자동화 하기 위한 CI Workflows 를 구현했습니다.
- Github Actions 를 활용하여 Master 브랜치에 CD 환경 구축
  - Master 브랜치에 push 될 때, Release 인지 아니면 다른 이유로 push 된 것인지 1차적으로 검사하기 위해 커밋 메세지를 검사합니다.
  - 'Release: X.X.X' 규칙으로 작성된 메세지라면, 다음 작업들을 실행합니다.
